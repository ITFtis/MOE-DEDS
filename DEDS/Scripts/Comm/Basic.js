...
function changetypeposition(...parts) {
    ...
    parts.forEach((part, index) => {
        ...
        if (index == 0) {
            ...
            // 創建一個新的標籤並插入到第一個元素前
            $('<div class="form-group field-container"><label class="col-sm-12 control-label">' + elements['$part' + index].find('label').text() + '</label></div>').insertBefore(elements['$part' + index]);
            ...
            //改變元素class讓他同一行
            elements['$part' + index].removeClass('col-md-12').addClass('col-md-6');
            elements['$part' + index].find('label').removeClass('col-sm-12').addClass('col-sm-3').addClass('semicolon');
            ...
            elements['$part' + index].find('label').removeClass('col-sm-12').addClass('col-sm-2').addClass('hashtag');
    ...
    var $row = $('<div class="row col-12"></div>').insertBefore(elements['$part1']);//將全部包再row裡面，變成一行
